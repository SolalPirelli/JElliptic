...
define(["require", "exports", "BigInteger"], function(require, exports, BigInteger) {
    ...
    (function (BigIntegerTests) {
        ...
        function add(s1, s2, result) {
            ...
            op("sub", result, s2, s1, function (b1, b2) {
                return b1.sub(b2);
            });

            if (s1 != s2) {
                op("add", s2, s1, result, function (b1, b2) {
                    return b1.add(b2);
                });
                op("sub", result, s1, s2, function (b1, b2) {
                    return b1.sub(b2);
                });
            }
        }

        function mul(s1, s2, result) {
            op("mul", s1, s2, result, function (b1, b2) {
                return b1.mul(b2);
            });

            if (s1 != s2) {
                op("mul", s2, s1, result, function (b1, b2) {
                    return b1.mul(b2);
                });
            }

            if (s1 != "0") {
                op("div (via divRem)", result, s1, s2, function (b1, b2) {
                    return b1.divRem(b2)[0];
                });
            }
            if (s1 != s2 && s2 != "0") {
                op("div (via divRem)", result, s2, s1, function (b1, b2) {
                    return b1.divRem(b2)[0];
                });
            }
        }
        ...
        function mod(s1, s2, result) {
            op("mod (via divRem)", s1, s2, result, function (b1, b2) {
                return b1.divRem(b2)[1];
            });
        }

        function modInverse(s1, s2, result) {
            op("modInverse", s1, s2, result, function (b1, b2) {
        ...
        function run() {
            ...
            add("843654783738219391462891409156201482963598234021939235792375230490324365", "96758932056432684895346825495765794382534257436257190023854239555353", "843751542670275824147786755981697248757980768279375492982399084729879718");
            ...
            add("249731758160310955319", "1794566138064068726342075845", "1794566387795826886653031164");
            add("843558024806162958777996062330705717169215699764502978602351376250769012", "96758932056432684895346825495765794382534257436257190023854239555353", "843654783738219391462891409156201482963598234021939235792375230490324365");
            ...
            mul("-1", "843654783738219391462891409156201482963598234021939235792375230490324365", "-843654783738219391462891409156201482963598234021939235792375230490324365");
            mul("235792375230490324365", "257190023854239555353", "60643446610177610597233595968726342075845");
            mul("201482963598234021939235792375230490324365", "495765794382534257436257190023854239555353", "99888361502825722750625541991816639206337292740267966437728204233595968726342075845");
            mul("843654783738219391462891409156201482963598234021939235792375230490324365", "96758932056432684895346825495765794382534257436257190023854239555353", "81631135898810780470790196457358272631935994421889038215604253548418790152344523969837735095109530292740267966437728204233595968726342075845");
        ...
        BigIntegerTests.run = run;
    })(BigIntegerTests || (BigIntegerTests = {}));

    
    return BigIntegerTests;
});
//# sourceMappingURL=BigIntegerTests.js.map
